...
#### we need to create EBS volume sepertely inside the cluster

**EBS-CSI Driver add for the cluster**
```
...
module "irsa_ebs_csi" {
  ...
  role_name             = "${var.cluster-name}-ebs-csi-irsa"
...
HPA is used to scale pods if the required condition is passed. first we need to install metrics server in the cluster.
...
delete the cluster using terraform
```
